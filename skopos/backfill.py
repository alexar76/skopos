...
def backfill_ua_and_referer(db_path: str, limit: int = 200000) -> int:
    ...
    cur.execute(
        ...
        WHERE log_source LIKE 'file:%'
          AND (ua_browser IS NULL OR ua_os IS NULL OR ua_device IS NULL OR ua_is_bot IS NULL)
        ORDER BY id DESC
        LIMIT {int(limit)}
        """
    )
...
def backfill_countries(db_path: str, mmdb_path: str | None = None, limit: int = 250000) -> int:
    con = connect(db_path)
    init_db(con)
    geo = GeoIPResolver(mmdb_path, api_fallback=True)
...
def backfill_all(db_path: str, mmdb_path: str | None = None) -> dict[str, int]:
    return {
