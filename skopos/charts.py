...
from .i18n import t
from .themes import apply_chart_theme, chart_layout_kwargs, get_active_theme, plotly_hover_bg
...
def _apply_donut(fig: go.Figure, title: str, height: int = 520) -> go.Figure:
    ...
    if fig.data:
        fig.update_traces(
            ...
            pull=[0.02] * n_slices,
            hoverlabel=dict(bgcolor=hover_bg, font_color=th.chart_font),
        )
    return fig
...
def chart_status_codes(df: pd.DataFrame, *, locale: str = "en") -> go.Figure:
    ...
    tmp["class"] = tmp["status"].astype(int).floordiv(100).astype(str) + "xx"
