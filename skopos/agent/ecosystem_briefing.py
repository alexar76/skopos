...
from skopos.agent.providers import ChatMessage, LLMProviderError, _is_reasoning_model, chat_completion
...
def traffic_snapshot_from_df(df: pd.DataFrame | None) -> TrafficSnapshot | None:
    ...
    status = df["status"] if "status" in df.columns else pd.Series(dtype=int)
    errors = status[status >= 500].count() if not status.empty else 0
