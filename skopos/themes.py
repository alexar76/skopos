...
_PLOTLY_TEMPLATES_READY = False


def ensure_plotly_templates() -> None:
    """Register SKOPOS Plotly templates with readable hoverlabel for every theme.

    Stock ``plotly_white`` / ``plotly_dark`` leave hoverlabel bgcolor unset; Plotly
    then paints a near-white unified hover box. Combined with light ``chart_font`` on
    dark UIs that yields invisible tooltip text ("country=… value=…") everywhere.
    """
    global _PLOTLY_TEMPLATES_READY
    if _PLOTLY_TEMPLATES_READY:
        return
    try:
        import copy
    ...
    for theme in THEMES.values():
        base_name = "plotly_white" if is_light_theme(theme.id) else "plotly_dark"
        payload = copy.deepcopy(pio.templates[base_name].to_plotly_json())
        layout = payload.setdefault("layout", {})
        layout["hoverlabel"] = {
            "bgcolor": theme_surface_bg(theme),
            "bordercolor": theme.accent,
            "align": "left",
            "font": {
                "family": "Inter, system-ui, sans-serif",
                "size": 13,
                "color": _plotly_hover_font(theme),
            },
        }
        layout["font"] = {
            "family": "Inter, system-ui, sans-serif",
        ...
        layout["paper_bgcolor"] = theme.chart_paper_bg
        layout["plot_bgcolor"] = theme.chart_plot_bg
        pio.templates[f"skopos_{theme.id}"] = payload

    _PLOTLY_TEMPLATES_READY = True


def skopos_plotly_template(theme: Theme) -> str:
    """Return the registered template name for ``theme`` (registers on first use)."""
    ensure_plotly_templates()
    return f"skopos_{theme.id}"
...
"""
  ...
  /* Theme the "Running… / _load_df(…)" status pill on FIRST PAINT so it never
     flashes Streamlit's default dark toast over a light theme (the brown bar). */
  html[data-skopos-theme="{tid}"] [data-testid="stStatusWidget"],
  html[data-skopos-theme="{tid}"] [data-testid="stConnectionStatus"] {{
    background: {pal["status_bg"]} !important;
  ...
  html[data-skopos-theme="{tid}"] [data-testid="stConnectionStatus"] * {{
    color: {pal["text"]} !important;
    fill: {pal["text"]} !important;
  }}
  html[data-skopos-theme="{tid}"] [data-testid="stStatusWidget"] svg,
  html[data-skopos-theme="{tid}"] [data-testid="stConnectionStatus"] svg {{
...
(function() {{
  ...
  function applySidebarWidth() {{
    if (isMobileViewport()) {{
      var sbMobile = document.querySelector('section[data-testid="stSidebar"]');
      if (sbMobile) {{
        sbMobile.style.removeProperty("width");
        sbMobile.style.removeProperty("min-width");
        sbMobile.style.removeProperty("max-width");
        var inner = sbMobile.querySelector(":scope > div");
        if (inner) {{
  ...
  try {{
    ...
    if (!isMobileViewport()) {{
      ...
      document.documentElement.classList.remove("skopos-sidebar-collapsed");
      document.documentElement.classList.remove("skopos-sidebar-expanded");
    }}
    applySidebarWidth();
    if (shells[id]) {{
...
<style id="stats-widget-theme">
  ...
  /* Spinner / progress — large enough to see in the empty analytics well */
  ...
  /* Text inputs — single inset border on inner shell only (avoid double frame) */
  .stApp div.element-container:has([data-testid="stTextInput"]),
...
def build_login_gate_css(theme: Theme) -> str:
    """Centered login card — one panel, no empty column chrome."""
    th = theme
    pal = _widget_palette(theme)
    widget_text = pal["widget_text"]
    widget_muted = pal["widget_muted"]
    return f"""
<style id="skopos-login-gate">
  .stApp:has(.skopos-login-page-marker) section[data-testid="stSidebar"] {{
    display: none !important;
  }}
  .stApp:has(.skopos-login-page-marker) [data-testid="stToolbar"],
  .stApp:has(.skopos-login-page-marker) [data-testid="stToolbarAppName"] {{
    display: none !important;
  }}
  .stApp:has(.skopos-login-page-marker) .main .block-container {{
    max-width: 26rem !important;
    width: min(26rem, calc(100vw - 2rem)) !important;
    margin: clamp(2.5rem, 10vh, 5rem) auto 2rem !important;
    padding: 2.1rem 2rem 2.35rem !important;
    background: {th.card_bg} !important;
    border: 1.5px solid {th.card_border} !important;
    border-radius: 18px !important;
    box-shadow: {th.card_shadow}, 0 18px 42px rgba(0, 0, 0, 0.28) !important;
    overflow: visible !important;
  }}

  .stApp:has(.skopos-login-page-marker) .main [data-testid="stVerticalBlockBorderWrapper"],
  .stApp:has(.skopos-login-page-marker) .main [data-testid="stForm"],
  .stApp:has(.skopos-login-page-marker) .main [data-testid="stForm"] > div,
  .stApp:has(.skopos-login-page-marker) .main [data-testid="stForm"] [data-testid="stVerticalBlock"],
  .stApp:has(.skopos-login-page-marker) .main [data-testid="stForm"] [data-testid="stTextInput"],
  .stApp:has(.skopos-login-page-marker) .main div.element-container:has([data-testid="stTextInput"]) {{
    border: none !important;
    box-shadow: none !important;
    background: transparent !important;
  ...
  .stApp:has(.skopos-login-page-marker) .main [data-testid="stVerticalBlockBorderWrapper"]:hover {{
    transform: none !important;
    box-shadow: none !important;
  }}

  .stApp:has(.skopos-login-page-marker) .main [data-testid="stExpander"] details {{
    border: 1px solid {th.border} !important;
    border-radius: 12px !important;
    box-shadow: none !important;
    background: color-mix(in srgb, {th.card_bg} 88%, {th.accent} 12%) !important;
    margin: 0.85rem 0 1.1rem !important;
  }}

  .stApp:has(.skopos-login-page-marker) .main h3 {{
  ...
  .stApp:has(.skopos-login-page-marker) .main [data-testid="stCaptionContainer"] {{
    text-align: center !important;
    margin-bottom: 0.35rem !important;
  }}
  .stApp:has(.skopos-login-page-marker) .main [data-testid="stCaptionContainer"] p {{
    color: {widget_muted} !important;
  ...
  .stApp:has(.skopos-login-page-marker) .main [data-testid="stTextInput"] [data-baseweb="input"],
  .stApp:has(.skopos-login-page-marker) .main [data-testid="stForm"] [data-testid="stTextInput"] [data-baseweb="input"] {{
    border: none !important;
    box-shadow: none !important;
    background: transparent !important;
    overflow: visible !important;
  }}
  .stApp:has(.skopos-login-page-marker) .main [data-testid="stTextInput"] [data-baseweb="input"] > div,
  .stApp:has(.skopos-login-page-marker) .main [data-testid="stForm"] [data-testid="stTextInput"] [data-baseweb="input"] > div {{
    background-color: {pal["input_inset_bg"]} !important;
    border: 1.5px solid {pal["input_inset_border"]} !important;
    border-radius: 10px !important;
    min-height: 2.85rem !important;
  ...
  .stApp:has(.skopos-login-page-marker) .main [data-testid="stFormSubmitButton"] > button {{
  ...
  .stApp:has(.skopos-login-page-marker) .main [data-testid="stFormSubmitButton"] > button p,
  .stApp:has(.skopos-login-page-marker) .main [data-testid="stFormSubmitButton"] > button span {{
    opacity: 1 !important;
    color: inherit !important;
  }}

  /* Hide chrome columns on login, but never hide the form submit row. */
  .stApp:has(.skopos-login-page-marker) .main [data-testid="stHorizontalBlock"]:not(:has([data-testid="stFormSubmitButton"])) {{
    display: none !important;
  }}
</style>
"""
...
<style id="stats-sidebar-layout">
  ...
  section[data-testid="stSidebar"] div.element-container:has([data-testid="stTextInput"]) {{
    overflow: visible !important;
  ...
  @media (min-width: 769px) {{
    ...
    section[data-testid="stSidebar"],
    section[data-testid="stSidebar"] > div {{
      width: var(--skopos-sidebar-expanded-width) !important;
      min-width: var(--skopos-sidebar-expanded-width) !important;
      max-width: var(--skopos-sidebar-expanded-width) !important;
      transform: none !important;
...
def build_app_css(theme: Theme) -> str:
    ...
    return f"""
<style>
  /* Local; metis-nginx serves /fonts/ on this host. See scripts/selfhost_google_fonts.py. */
  ...
  /* Inputs, selects, multiselects — text fields use inset shell in build_widget_css */
  ...
  .skopos-page-loading-wheel {{
    width: 1.7rem;
    height: 1.7rem;
    flex: 0 0 1.7rem;
    border: 3px solid color-mix(in srgb, {th.accent} 28%, transparent);
    border-top-color: {th.accent};
    border-radius: 50%;
    animation: skoposSpin 0.7s linear infinite;
  }}
  ...
  /* Sidebar expanders already have a card frame — drop nested border wrappers */
  ...
  /* Fenced code blocks rendered via st.markdown (docs, help text). These are
     plain <pre><code> inside stMarkdownContainer and are NOT caught by the
     stCode rules above; without this the global `span {{ color: inherit }}`
     collapses Streamlit's syntax-highlight token colors to the page text
     color, leaving unreadable text on the dark code background. */
  ...
  /* Inline `code` in markdown / expander labels (SSH host badges).
     Exclude fenced-block code: Streamlit nests it as pre>div>code, so the
     direct parent is a <div> (not <pre>) and a naive :not(pre) > code would
     wrongly paint the block <code> with the light inline background. */
  .stApp [data-testid="stMarkdownContainer"] :not(pre) > code:not(pre code),
  ...
  @media (max-width: 768px) {{
    ...
    /* Do not force collapsed rail widths on mobile — drawer CSS owns this. */
    div[data-testid="stVerticalBlockBorderWrapper"] {{
...
def build_agent_widget_css(theme: Theme) -> str:
    ...
    fab_btn = (
        'section.main [data-testid="stFragment"]:has(.stats-agent-anchor) [data-testid="stButton"] > button, '
    ...
    return f"""
<style>
  /* Agent mount slot — zero layout footprint; never collapse page-level vertical blocks */
  section.main div.element-container:has(> .skopos-agent-mount),
  section.main div.element-container:has(> .skopos-agent-float-root) {{
    height: 0 !important;
    min-height: 0 !important;
    max-height: 0 !important;
    overflow: visible !important;
  ...
  section.main div.element-container:has(> .stats-agent-root) {{
    height: 0 !important;
    min-height: 0 !important;
    overflow: visible !important;
    margin: 0 !important;
  ...
  /* FAB — only the JS-tagged chat button (never broad :has() on page blocks) */
  button.skopos-agent-fab {{
  ...
  /* Legacy innermost-block — layout neutral; FAB uses button.skopos-agent-fab only */
  ...
  /* Portaled to body via agent_portal.js — selectors must not rely on section.main */
  body > .skopos-agent-portal.skopos-agent-portal--open {{
    background: rgba(2, 6, 23, 0.95) !important;
    border: 1px solid rgba(255, 255, 255, 0.1) !important;
    border-radius: 1rem !important;
  ...
  body > .skopos-agent-portal.skopos-agent-portal--closed {{
    background: transparent !important;
    border: none !important;
    box-shadow: none !important;
  }}
...
def plotly_hover_font(theme: Theme) -> str:
    """High-contrast hover text — paired with :func:`plotly_hover_bg`."""
    return _plotly_hover_font(theme)


def _plotly_hover_bg(theme: Theme) -> str:
    """Solid hover box for every theme (never fall back to white on dark UIs)."""
    # Reuse the same solid surfaces as contrast checks so ocean/aurora/etc. are covered.
    return theme_surface_bg(theme)


def _plotly_hover_font(theme: Theme) -> str:
    """Readable text on the hover box (light text on dark surfaces, vice versa)."""
    if is_light_theme(theme.id):
        return "#1a1a1a"
    return "#f4f7fb"
...
def chart_layout_kwargs(theme: Theme | None = None) -> dict:
    ...
    hover_bg = _plotly_hover_bg(th)
    hover_font = _plotly_hover_font(th)
    return dict(
        ...
        hoverlabel=dict(
            ...
            font=dict(size=13, family="Inter, system-ui, sans-serif", color=hover_font),
        ),
...
def apply_chart_theme(fig, theme: Theme | None = None):
    ...
    fig.update_layout(
        ...
        hoverlabel=dict(
            bgcolor=_plotly_hover_bg(th),
            bordercolor=th.accent,
            font=dict(size=13, family="Inter, system-ui, sans-serif", color=_plotly_hover_font(th)),
        ),
    )
    fig.update_traces(
