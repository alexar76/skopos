...
from skopos.remediation.clients import AgentDeployer, FactoryClient, MomusClient
...
class Conductor:
    def __init__(self, config: RemediationConfig | None = None):
        ...
        self.deployer = AgentDeployer(dry_run=self.cfg.dry_run)
    ...
    async def _run(self, job: Job) -> Job:  # type: ignore[name-defined]
        while job.attempts < self.cfg.max_attempts:
            ...
            if not dispatch.get("accepted"):
                job.transition(JobState.FAILED, f"deploy rejected: {dispatch.get('error') or dispatch.get('reason')}")
                self.store.upsert(job)
                continue
