...
with tab_hub:
    ...
    hub_rate = parse_range_matrix(
        query_range("sum by (capability) (rate(aimarket_hub_invokes_total[5m]))", hours=1.0)
    )
    ...
    heat_rows = parse_instant_vector(
        query_instant("sum by (capability, result) (rate(aimarket_hub_invokes_total[15m]))")
    )
...
with tab_factory:
    ...
    factory_series = parse_range_matrix(query_range('up{job="aicom"}', hours=6.0, step="1m"))
...
with tab_mesh:
    ...
    metis_series = parse_range_matrix(
        query_range('up{job="metis"} or on() vector(0)', hours=6.0, step="1m")
    )
    if not metis_series:
        metis_series = parse_range_matrix(query_range("metis_up", hours=6.0, step="1m"))
