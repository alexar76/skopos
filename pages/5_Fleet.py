...
from skopos.ui_transport import render_agent_panel, render_transport_picker
...
from skopos.ui_refresh import render_section_refresh
...
def _conn_info(server: ServerConfig) -> SSHConnInfo:
    """Connection details, via the one builder that knows about transports.

    Building this by hand left ``forced_command`` unset, so a connection test
    against an agent-ssh host sent a shell script to a key that accepts two
    words — and reported the refusal as a broken host.
    """
    from skopos.remote_ops import ssh_info

    return ssh_info(server)
...
render_section_refresh("fleet_db", locale)
...
render_section_refresh("fleet_scan", locale)
...
render_section_refresh("fleet_telegram", locale)
...
render_section_refresh("fleet_keys", locale)
...
render_section_refresh("fleet_servers", locale)
...
for idx, server in enumerate(list(servers)):
    ...
    with st.expander(f"🖥️ **{server.name}** — `{server.ssh.user}@{server.ssh.host}:{server.ssh.port}`", expanded=False):
        choice = render_transport_picker(server, key_prefix=f"srv{idx}")
        uses_ssh = choice.transport != "agent-push"
        st.markdown("---")
        ...
        with c1:
            ...
            host = st.text_input(
                T(ctx, "settings.field_host"),
                value=server.ssh.host,
                key=f"srv_host_{idx}",
                help=None if uses_ssh else "Used to label this host's traffic. SKOPOS never connects to it.",
            )
            user = st.text_input(
                T(ctx, "settings.field_user"),
                value=server.ssh.user,
                key=f"srv_user_{idx}",
                disabled=not uses_ssh,
            )
        with c2:
            port = st.number_input(T(ctx, "settings.field_port"), value=int(server.ssh.port), min_value=1, max_value=65535, key=f"srv_port_{idx}", disabled=not uses_ssh)
            key_path = st.text_input(T(ctx, "settings.field_key_path"), value=server.ssh.key_path or "~/.ssh/id_ed25519", key=f"srv_key_{idx}", disabled=not uses_ssh)
            pass_env = st.text_input(T(ctx, "settings.field_pass_env"), value=server.ssh.key_passphrase_env or "SKOPOS_SSH_KEY_PASSPHRASE", key=f"srv_pass_{idx}", disabled=not uses_ssh)
        with c3:
        ...
        servers[idx] = draft_server_from_form(
            ...
            ssh_mode=choice.ssh_mode,
            known_hosts_path=server.ssh.known_hosts_path,
        )

        render_agent_panel(servers[idx], config_path=config_path, key_prefix=f"srv{idx}")
...
render_section_refresh("fleet_add_server", locale)
...
with save_left:
    if st.button(T(ctx, "settings.save_config"), type="primary", use_container_width=True):
        try:
            ...
            st.cache_data.clear()
            from skopos.fleet_warmup import warm_fleet_after_save

            with st.spinner(T(ctx, "settings.warmup_running")):
                warm = warm_fleet_after_save(new_cfg, previous_servers=cfg.servers)
            ok_scans = sum(1 for s in warm.scans if s.ok)
            fail_scans = [s for s in warm.scans if not s.ok]
            if warm.server_names:
                st.success(
                    T(
                        ctx,
                        "settings.warmup_ok",
                        servers=", ".join(warm.server_names),
                        collected=len(warm.collect),
                        scanned=ok_scans,
                    )
                )
                for bad in fail_scans[:5]:
                    st.warning(f"{bad.server_name}: {bad.error or 'scan failed'}")
            else:
