...
from skopos.ui_refresh import bump_refresh, refresh_nonce, render_section_refresh
...
def _load_security(
    db_target: str, server_names: tuple[str, ...], refresh_nonce: int = 0
...
@st.cache_data(show_spinner=False)
def _load_knocks(
    db_target: str, server_names: tuple[str, ...], hours: int = 168, refresh_nonce: int = 0
...
if st.button(T(ctx, "common.refresh"), key="security_page_refresh", help=T(ctx, "common.refresh_hint")):
    bump_refresh("security")
    bump_refresh("security_knocks")
    bump_refresh("posture")

_db = resolve_db_target(cfg)
snapshots, findings_map = _load_security(_db, known, refresh_nonce("security"))
...
with tab_score:
    render_section_refresh("security_score", ctx.locale)
    render_posture_panel(ctx.posture, locale=ctx.locale)

with tab_report:
    render_section_refresh("security_report", ctx.locale)
...
with tab_ov:
    render_section_refresh("security_overview", ctx.locale)
...
with tab_ports:
    render_section_refresh("security_ports", ctx.locale)
    for snap in snapshots:
        p = snap["payload"]
...
with tab_knocks:
    render_section_refresh("security_knocks", ctx.locale)
    ...
    knock_df, actor_df = _load_knocks(_db, tuple(knock_servers), 168, refresh_nonce("security_knocks"))
...
with tab_res:
    render_section_refresh("security_resources", ctx.locale)
    for snap in snapshots:
        p = snap["payload"]
...
with tab_audit:
    render_section_refresh("security_audit", ctx.locale)
    for snap in snapshots:
...
with tab_3d:
    render_section_refresh("security_3d", ctx.locale)
    for snap in snapshots:
...
with tab_agent:
    render_section_refresh("security_agent", ctx.locale)
