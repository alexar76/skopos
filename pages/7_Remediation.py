...
def _load_store(db_target: str, refresh_nonce: int = 0) -> dict:
...
render_section_refresh("remediation", locale)

data = _load_store(resolve_db_target(ctx.cfg), refresh_nonce("remediation"))
...
if rows:
    ...
    render_section_refresh("remediation_board", locale)
    st.caption(L(
...
render_section_refresh("remediation_a2a", locale)
st.caption(L(
