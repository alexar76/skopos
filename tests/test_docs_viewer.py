...
def test_doc_section_labels_keys_match_guide_sections():
    from skopos.doc_i18n import doc_section_labels
    from skopos.docs_viewer import GUIDE_SECTIONS

    for loc in ("en", "ru", "zh", "es"):
        labels = doc_section_labels(loc)
        for _slug, key in GUIDE_SECTIONS:
            assert key in labels, f"{loc}: missing {key}"
            assert labels[key].strip(), f"{loc}: empty label for {key}"
