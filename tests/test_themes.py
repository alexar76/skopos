...
def test_chart_layout_hover_has_contrast_on_all_themes():
    """Hover text must stay readable — dark themes must not fall back to white hover boxes."""
    from skopos.themes import DARK_THEME_IDS, LIGHT_THEME_IDS, contrast_ratio, ensure_plotly_templates

    import plotly.io as pio

    ensure_plotly_templates()
    for tid in THEME_ORDER:
        assert f"skopos_{tid}" in pio.templates
        layout = chart_layout_kwargs(THEMES[tid])
        assert layout["template"] == f"skopos_{tid}"
        bg = layout["hoverlabel"]["bgcolor"]
        font = layout["hoverlabel"]["font"]["color"]
        if tid in DARK_THEME_IDS:
            assert bg.lower() != "#ffffff", tid
        if tid in LIGHT_THEME_IDS:
            assert font.lower() in {"#1a1a1a", "#202124", "#1c2333"} or contrast_ratio(font, bg) >= 4.5
        else:
            assert contrast_ratio(font, bg) >= 4.5, (tid, font, bg)
        # Template itself must bake the same hoverlabel (unified hover reads template).
        tpl_hover = pio.templates[f"skopos_{tid}"].layout.hoverlabel
        assert tpl_hover.bgcolor == bg
        assert tpl_hover.font.color == font
...
def test_topbar_css_styles_running_widget():
    ...
    assert "topbar_settings" not in css  # i18n key, not in CSS
    assert ".skopos-topbar-link .label" in css
...
def test_app_css_includes_page_loading_spinner():
    css = build_app_css(THEMES["light"])
    assert "skopos-page-loading" in css
    assert "skoposSpin" in css
...
def test_topbar_css_includes_mobile_menu():
    from skopos.ui_topbar import build_topbar_css, build_topbar_js

    css = build_topbar_css(THEMES["light"])
    js = build_topbar_js()
    assert "skopos-mobile-menu-btn" in css
    assert "skopos-mobile-sidebar-backdrop" in css
    assert "translateX(-105%)" in css
    assert "skopos-mobile-sidebar-open" in css
    assert "setMobileSidebarOpen" in js
    assert "window.parent" in js
    assert "toggleSkoposMobileMenu" in js
    assert "__skoposTopbarClickBound" in js
...
def test_agent_widget_css_targets_innermost_block_only():
    ...
    assert "skopos-agent-mount" in agent_css
    assert "button.skopos-agent-fab" in agent_css
    assert "stats-agent-fab-slot" in agent_css
    assert "position: fixed" in agent_css
    assert 'stVerticalBlock"]:has(.stats-agent-anchor)' not in agent_css
