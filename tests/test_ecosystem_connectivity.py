...
def test_basanos_and_themis_segments_by_vhost():
    assert ecosystem_segment("/health", host="basanos.modelmarket.dev") == "basanos"
    assert ecosystem_segment("/ui/", host="basanos.modelmarket.dev") == "basanos"
    assert ecosystem_segment("/health", host="themis.modelmarket.dev") == "themis"
    assert ecosystem_segment("/ui/", host="themis.modelmarket.dev") == "themis"
    # Without explicit rules these vhosts would fall through to generic "hub".
    assert ecosystem_segment("/health", host="modelmarket.dev") == "hub"

    basanos_host = infer_host("/memos", ecosystem_segment="basanos", server_name="oracle")
    assert basanos_host == "basanos.modelmarket.dev"
    themis_host = infer_host("/health", ecosystem_segment="themis", server_name="factory")
    assert themis_host == "themis.modelmarket.dev"
