...
def capture(base_url: str, password: str, *, gallery_only: bool = False) -> None:
    ...
    with sync_playwright() as p:
        ...
        # Force English UI before any readme hero capture (prod default may be RU).
        _scroll_sidebar(page)
        lang_box = page.locator('section[data-testid="stSidebar"] [data-testid="stSelectbox"]').nth(0)
        lang_box.locator('[data-baseweb="select"]').click()
        page.locator('[data-baseweb="popover"] [role="option"]').nth(0).click()
        page.wait_for_timeout(3200)
