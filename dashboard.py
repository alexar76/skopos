...
with st.sidebar.expander(T(ctx, "analytics.collection")):
    ...
    if st.button(T(ctx, "settings.backfill"), use_container_width=True):
        with st.spinner(T(ctx, "analytics.updating")):
            st.success(backfill_all(db_target, mmdb_path=cfg.geoip_mmdb_path))
        st.cache_data.clear()
        st.rerun()
...
m3.metric(T(ctx, "analytics.countries"), f"{kpis.countries:,}")
m4.metric(T(ctx, "analytics.pages"), f"{kpis.pages:,}")
m5.metric(T(ctx, "analytics.hosts"), f"{kpis.hosts:,}")
