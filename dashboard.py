...
from skopos.ui import hero, page_loading, plot, plot_fullscreen, section_head
...
host_opts, country_opts = _cached_filter_options(
    db_target, since_iso, until_iso, known_servers, analytics_nonce
)
...
# Cache helpers hide Streamlit's default "Running _cached_*()" pill, and the
# SKOPOS chrome hides stHeader (where the native status widget lives) — so first
# paint used to sit on an empty chart well with no spinner at all.
with page_loading(T(ctx, "analytics.loading")):
    has_traffic = _cached_has_traffic(db_target, since_iso, until_iso, known_servers, analytics_nonce)
    kpis = _cached_kpis(db_target, since_iso, until_iso, known_servers, fk, filters, analytics_nonce)
    countries_df = _cached_country_stats(
        db_target, since_iso, until_iso, known_servers, fk, filters, analytics_nonce
    )
    snap = _cached_traffic_snapshot(
        db_target, since_iso, until_iso, known_servers, fk, filters, analytics_nonce
    )
...
inject_tab_deeplink(["overview", "geo", "audience", "content", "sources", "journal", "system"])
...
with tab_overview:
    ov_n = render_section_refresh("analytics_overview", loc)
    with st.container(border=True):
        gran = st.radio(
...
with tab_geo:
    geo_n = render_section_refresh("analytics_geo", loc)
    ...
    with st.container(border=True):
        plot(chart_countries_timeline(_q("country_hourly", geo_n, metric="requests", top_n=6), metric="requests", locale=loc), key="geo_tl_req")
    ...
    with st.container(border=True):
        plot(chart_countries_timeline(_q("country_hourly", geo_n, metric="visitors", top_n=6), metric="visitors", locale=loc), key="geo_tl_vis")

    with st.container(border=True):
        plot(chart_countries_by_host(_q("host_country", geo_n, metric="requests"), metric="requests", locale=loc), key="geo_host")
...
with tab_audience:
    aud_n = render_section_refresh("analytics_audience", loc)
    ...
    with st.container(border=True):
        plot(chart_donut_dimension(_q("top", aud_n, column="ua_os", top_n=20), "ua_os", T(ctx, "analytics.chart_os"), top_n=8, locale=loc), key="aud_os")

    section_head(T(ctx, "analytics.section_devices"))
    with st.container(border=True):
        plot(chart_donut_dimension(_q("top", aud_n, column="ua_device", top_n=20), "ua_device", T(ctx, "analytics.chart_devices"), top_n=6, locale=loc), key="aud_device")

    section_head(T(ctx, "analytics.section_ips"))
    with st.container(border=True):
        plot(chart_top_dimension(_q("top", aud_n, column="remote_addr", top_n=15), "remote_addr", T(ctx, "analytics.chart_top_ips"), top_n=15), key="aud_ip")

with tab_content:
    cnt_n = render_section_refresh("analytics_content", loc)
    with st.container(border=True):
        plot(chart_treemap_pages(_q("treemap", cnt_n), locale=loc), key="cnt_tree")
    c1, c2 = st.columns(2, gap="large")
    with c1:
        with st.container(border=True):
            plot(chart_top_dimension(_q("top", cnt_n, column="path", top_n=15), "path", T(ctx, "analytics.chart_popular_paths"), top_n=15), key="cnt_paths")
    with c2:
        with st.container(border=True):
            plot(chart_top_dimension(_q("top", cnt_n, column="host", top_n=12), "host", T(ctx, "analytics.chart_popular_hosts"), top_n=12), key="cnt_hosts")
    with st.container(border=True):
        plot(chart_ecosystem(_q("top", cnt_n, column="ecosystem_segment", top_n=30), locale=loc), key="cnt_eco")

with tab_sources:
    src_n = render_section_refresh("analytics_sources", loc)
    with st.container(border=True):
        plot(chart_top_dimension(_q("top", src_n, column="referer_domain", top_n=15), "referer_domain", T(ctx, "analytics.chart_referers"), top_n=15), key="src_ref")
...
with tab_visitors:
    vis_n = render_section_refresh("analytics_journal", loc)
...
with tab_system:
    sys_n = render_section_refresh("analytics_system", loc)
    _ = sys_n
    con = connect_for_config(cfg)
